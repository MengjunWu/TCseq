...
#'
#' This function merges genomic coordinates of a given data frame or reads in BED files (e.g. generated from
#' a peak caller) under given directory and merge genomic regions that have overlapping genomic intervals into
#' a single feature. The single feature represents the widest genomic interval that covers all merged regions.
#'
#' @param data a data frame containg coordinates information of peaks to be merged. Columns of the data frame
#' should be consistent with the BED format where the first column is the name of the chromosome, the second
#' column is the starting position and the third column is the ending position.
#'
#' @param dir character string giving the directory where BED files are stored. If \code{data} is not given,
#' the function will reads in the BED files under \code{code}.
#'
#' @param pattern an \code{\link{regular expression}}, only files that have names match the regular expression
#' will be read in.
#'
#' @param merge logical indicating whether to merge overlapped regions or not. If False, regions are simply
#' combined.
#'
#' @param overlap a numberic value giving the least number of base(s) two regions should overlap when merging
#' them.
#'
#' @param ratio a numberic value giving the thresold of overlapping ratio between two regions to merge them.
#' See '\code{Details}'
#' below for the definition of the overlapping ratio.
#'
#' @return a data frame with four columns: \code{chr}, \code{start}, \code{stop}, \code{id}
#'
#' @details
...
#'
#' \eqn{a}, \eqn{b} are two genomic regions, \eqn{n} is the number of overlapping bases between region \eqn{a}
#' and region \eqn{b}.
#'
#' @author
#' Mengjun Wu, Lei Gu
